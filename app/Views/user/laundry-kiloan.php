...
<?= $this->section('title') ?>
<title>Mawar Laundry | laundry-kiloan</title>
<?= $this->endSection(); ?>
      ...
      >
        <div class="modal-dialog text-black">
          <div class="modal-content">
            ...
            <form action="<?= base_url('/laundry-kiloan') ?>" method="post">
      ...
      <div class="row" style="<?= count($transaksi) > 2 ? 'height: auto;':'height: 70vh;' ?>">
        <?php if(empty($transaksi)): ?>
          <div class="col-lg-12" style="color: #adb5bd;margin: auto auto;text-align: center;">
            <h4>Anda pernah belum melakukan transaksi laundry kiloan</h4>
          </div>
        <?php endif; ?>
        <?php foreach ($transaksi as $key => $value) : ?>
        <div class="col-lg-6">
          <div class="card mb-4 bg-white text-light text-black">
            <h5 class="card-header">Nomor Laundry : <?= 'LK'.$value->id_transaksi ?></h5>
            <div class="card-body">
              ...
              <?php endif; ?>
              <a href="<?= base_url('/'.$value->id_transaksi.'/detail-kiloan') ?>" class="btn btn-info"
                >DETAIL LAUNDRY</a
              >
            </div>
          </div>
