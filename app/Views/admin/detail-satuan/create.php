...
<section class="section">
          <div class="section-header">
            <div class="section-header-back">
              <a href="<?= base_url('admin/'.$id_transaksi.'/detail-satuan') ?>" class="btn"><i class="fa fa-arrow-left"></i></a>
            </div>
          ...
          <div class="section-body">
              <div class="card">
                ...
                <div class="card-body col-md-8">
                    <form action="<?= base_url('admin/detail-satuan') ?>" method="post" autocomplete="off">
                      <?= csrf_field() ?>
                      ...
                      <div class="form-group">
                        ...
                        <select name="barang" id="barang" value="<?= old('barang') ?>" class="form-control <?php if(!empty($validation['errors']['barang'])){echo($validation['errors']['barang']) ? 'is-invalid' : ''; }?>" autofocus>
                            <option disabled selected value>--Pilih Barang Laundry--</option>
                            <?php
                              foreach ($barang as $key => $value) :
                            ?>
                            <option value="<?= $value->id_barang ?>" <?= old('barang')==$value->id_barang?'selected':'' ?>><?= $value->nama_barang ?></option>
                            <?php endforeach; ?>
                        </select>
                        <div class="invalid-feedback">
                      ...
                      <div class="form-group">
                        ...
                        <input value="<?= old('jml_barang') ?>" type="number" name="jml_barang" class="form-control <?php if(!empty($validation['errors']['jml_barang'])){echo($validation['errors']['jml_barang']) ? 'is-invalid' : ''; }?>" autofocus>
                        <div class="invalid-feedback">
